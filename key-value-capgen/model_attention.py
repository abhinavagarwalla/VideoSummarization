...
class Attention(object):
	...
	def fflayer(self, tparams, state_below, options,
		...
		#print ('init ff layer', prefix, tparams[_p(prefix,'W')].shape)
	...
	def param_init_lstm_cond(self, options, params,
							 prefix='lstm_cond', nin=None, dim=None, dimctx=None, dimvalue=None, dim_key_add=None):
		if nin == None:
			nin = options['dim']
		if dim == None:
			dim = options['dim']
	...
	def lstm_cond_layer(self, tparams, state_below, options, prefix='lstm',
						mask=None, context=None, value=None, one_step=False,
						init_memory=None, init_state=None,
						init_memory_key=None, init_state_key=None,
						init_key_ctx=None, init_val_ctx=None,
		...
		assert value, 'Value must be provided for key-value Memory Networks'
		...
		Wk_att = tparams[_p(prefix,'Wk_att')]
		U_att = tparams[_p(prefix,'U_att')]
		c_att = tparams[_p(prefix, 'c_tt')]
		#print ('bvkjbvkd before selector')
		if options['selector']:
		...
		def _step(m_, x_, # sequences
			...
			print ('before alpha calculation')
			...
			#     ctx_ = sel_[:,None] * ctx_
			print ('after alpha calculation')
			preact = tensor.dot(h_, U)
			preact += x_
			preact += tensor.dot(vctx_, Wc)
	...
	def init_params(self, options):
		...
		# init_state, init_hidden state and memory cell
		...
		params = self.get_layer('lstm_cond')[0](options, params, prefix='decoder',
										   ...
										   dimctx=ctx_dim, dimvalue=value_dim)
		...
		if options['ctx2out']:
			params = self.get_layer('ff')[0](
				...
				nin=value_dim, nout=options['dim_word'])
	...
	def build_model(self, tparams, options):
		...
		val = tensor.tensor3('val', dtype='float32')
		val.tag.test_value = self.value_tv
		mask_val = tensor.matrix('mask_val', dtype='float32')
		mask_val.tag.test_value = self.value_mask_tv
		...
		#print ('bvsdkjvbkdlbvdvbe', ctx_mean.shape)
		...
		init_state_key = self.get_layer('ff')[1](
			tparams, ctx_mean, options, prefix='ff_state_key', activ='tanh')
		init_memory_key = self.get_layer('ff')[1](
			tparams, ctx_mean, options, prefix='ff_memory_key', activ='tanh')
		...
		extra = [probs, alphas]
		return trng, use_noise, x, mask, ctx, mask_ctx, val, mask_val, alphas, cost, extra

	def build_sampler(self, tparams, options, use_noise, trng, mode=None):
		# context: #annotations x dim
		print ('here1 in sampler')
		...
		val0 = tensor.matrix('val_sampler', dtype='float32')
		val_mask = tensor.vector('val_mask', dtype='float32')
	...
	def gen_sample(self, tparams, f_init, f_next, ctx0, ctx_mask, val0, val_mask, options,
		...
		next_state_key[-1] = next_state_key[-1].reshape([live_k, next_state_key[-1].shape[0]])
...
>>>>>>> Pushing Local File
				if validFreq != -1 and numpy.mod(uidx, validFreq) == 0:
					...
					t0_valid = time.time()
					alphas,_ = f_alpha(x, mask, ctx, ctx_mask, val, val_mask)
