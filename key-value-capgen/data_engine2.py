...
class Movie2Caption(object):
    ...
    def get_video_values(self, vidID):
        vals = self.VALUES[vidID]
        vals = self.get_sub_frames(vals)
        return vals
    ...
    def prepare_data_for_blue(self, whichset):
        ...
        vals = []
        captions = []
        def get_words(self, vidID):
            caps = self.CAP[vidID]
            caption = []
            for cap in caps:
                caption.append(cap['tokenized'].split(' '))
            return caption
        ...
        for i, vidID in enumerate(ids):
            ...
            feats.append(feat)
            vals.append(self.get_video_values(vidID))
            feat_mask = self.get_ctx_mask(feat)
            feats_mask.append(feat_mask)
            captions.append(get_words(self, vidID))
        return (numpy.asarray(feats)), feats_mask, (numpy.asarray(vals)), feats_mask, captions
    ...
    def load_data(self):
        if self.signature == 'youtube2text':
            ...
            self.FEAT = common.load_pkl('../y2t/video_keys.pkl')
            self.VALUES = common.load_pkl('../y2t/video_values.pkl')#dataset_path + 'FEAT_key_vidID_value_features.pkl')
...
def prepare_data(engine, idx):
    ...
    for i, ID in enumerate(IDs):
        ...
        vals = engine.get_video_values(vidID)
    ...
    if engine.maxlen != None:
        ...
        new_caps = []
        new_val_list = []
        for l, s, y, c, v in zip(lengths, seqs, feat_list, IDs, val_list):
    ...
    val_list = numpy.asarray(val_list)
