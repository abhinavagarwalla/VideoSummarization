...
def score_with_cocoeval(samples_valid, samples_test, engine):
    ...
    if samples_valid:
        ...
        print len(gts_valid), len(samples_valid)
...
def generate_sample_gpu_single_process(
    ...
    def sample(whichset):
        samples = []
        ctxs, ctx_masks, vals, val_masks, captions = engine.prepare_data_for_blue(whichset)
        for i, ctx, ctx_mask, val, val_mask, caption in zip(range(len(ctxs)), ctxs, ctx_masks, vals, val_masks, captions):
            print 'sampling %d/%d'%(i,len(ctxs))
            sample, score, _, _ = model.gen_sample(
                None, f_init, f_next, ctx, ctx_mask, val, val_mask, options,
    ...
    if whichset == 'valid' or whichset == 'both':
        ...
        with open('valid_samples.txt', 'w') as f:
            print >>f, '\n'.join(samples_valid)
